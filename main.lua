...
local function updatePlayer(dt)
  ...
  if love.keyboard.isDown('down') then dy = speed * dt elseif love.keyboard.isDown('up') then dy = -speed * dt end
  if love.keyboard.isDown('s') then dy1 = spd * dt elseif love.keyboard.isDown('w') then dy1 = -spd * dt end
...
local function drawBlocks()
  for v,block in ipairs(blocks) do
    drawBox(block, 1, 0, 0)
  end
end
